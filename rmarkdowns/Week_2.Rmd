...
(data = world) +
  geom_sf() +
  coord_sf(xlim = c(-77.05, -77.03), ylim = c(38.88, 38.89)) + # zoom into D.C. area
  labs(title = "Washington D.C.", subtitle = "Cherry Blossom Field in Tidal Basin") +
  theme_minimal()
...
ggplot() +
  geom_sf(data = world) +
  geom_sf(data = states) +
  coord_sf(xlim = c(-77.1, -76.9), ylim = c(38.85, 39), expand = FALSE) +
  ggtitle("Washington D.C.", subtitle = "Cherry Blossom Field in Tidal Basin") +
  theme_minimal()
```
...
# Another code type for global application:
```{r}
# Rivers (global)
rivers_world <- ne_download(scale = 10, type = "rivers_lake_centerlines", category = "physical", returnclass = "sf")
# Lakes (global)
lakes_world <- ne_download(scale = 10, type = "lakes", category = "physical", returnclass = "sf")
```
...
```{r}
ggplot() +
  geom_sf(data = world, fill = "#838b8b") +
  geom_sf(data = states, fill = "#00cdcd", size = 0.25) +
  geom_sf(data = lakes50, fill = "#00bfff", color = "black") +
  geom_sf(data = rivers50, fill = "#00bfff", size = 0.15) +
  coord_sf(xlim = c(-77.1, -76.9), ylim = c(38.85, 39), expand = FALSE) +
  ggtitle("Washington D.C.", subtitle = "Cherry Blossom Field in Tidal Basin") +
theme_minimal()
```
...
ggplot() +
  ...
  geom_sf(data = lakes50, fill = "#00bfff", color = "black") +
  geom_sf(data = rivers50, fill = "#00bfff", size = 0.15) +
  geom_sf(data = Washington_DC, fill = "red", color = "red", size = 5) +
  coord_sf(xlim = c(-77.1, -76.9), ylim = c(38.85, 39), expand = FALSE) +
  ggtitle("Washington D.C.", subtitle = "Cherry Blossom Field in Tidal Basin") +
  theme_minimal()
```

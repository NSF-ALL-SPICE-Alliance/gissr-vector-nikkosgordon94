...
# Also can be downloaded through github terminal if package not running through install tools. GitHub has a cope file for naturalearth packages, just make sure to download the install instead of the whole chunk
...
ggplot(data = world) +
    geom_sf() +
    ggtitle("Countries of the World", subtitle = "Created by Nikkos Miyasato")
...
```
# Created my own ggplot for the assignment
...
```{r}
ggplot() +
  geom_sf(data = world_without_Australia, fill = "#e2062c", color = "#000", size = 0.50) +
   ggtitle("Countries of the world", subtitle = "Without Australia") + theme_void()
```
...
ggplot() +
  geom_sf(data = world_without_Australia,
          aes(fill = as.factor(mapcolor7)),
          color = "#401D16", size = 0.20) +
  scale_fill_brewer(palette = "Paired") +
  guides(fill = "none") + coord_sf(crs = "+proj=ortho +lat_=0=21.21 +lon_0=-157.54") + ggtitle("Salt Lake, Hawai'i as the Center of the World", subtitle = "Orthographic Azimuthal")+
  theme_void()
```

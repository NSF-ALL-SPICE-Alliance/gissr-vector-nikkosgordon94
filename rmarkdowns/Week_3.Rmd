...
ggplot() +
  ...
  geom_sf_text(data = water$osm_polygons, aes(label = name), size = 2, hjust = 0) +
  coord_sf(xlim = c(-77.1197, -76.90937), ylim = c(38.79163, 38.99597), expand = FALSE)
```
...
streets <- bb %>%
  opq() %>%
  add_osm_feature(key = "highway", value = c("primary", "secondary", "tertiary", "motorway")) %>%
  osmdata_sf()
...
```{r}
ggplot() +
  geom_sf(data = place$osm_lines, inherit.aes = FALSE, color = "red", size = .5, alpha = .8) +
  geom_sf(data = streets$osm_lines, inherit.aes = FALSE, color = "#8b7500", size = .2, alpha = .5) +
  geom_sf(data = small_streets$osm_lines, inherit.aes = FALSE, color = "#ffd700", size = .2, alpha = .5) +
  theme_bw() +
  labs(
    title = "Residents of Washington D.C",
    x = "Longitude",
    y = "Latitude"
  )
```
